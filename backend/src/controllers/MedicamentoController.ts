...
export class MedicamentoController {
    ...
    try {
        ...
        if (!nome || !dosagem || !compartimento) {
            return res.status(400).json({ erro: "Nome, dosagem e compartimento são obrigatórios." });
        }   
...
async getById(req: Request, res: Response) {
    ...
    async update(req: Request, res: Response) {
        try {
            ...
            return res.status(500).json({ erro: "Erro ao atualizar medicamento. Verifique se o ID existe." });
        }
    }
