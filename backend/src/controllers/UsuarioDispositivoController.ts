...
export class UsuarioDispositivoController {
    ...
    try {
      ...
      if (
        ...
        return res.status(400).json({
          erro:
            "usuario_id, dispositivo_id e tipo_acesso são obrigatórios.",
        });
      }
      ...
      return res.status(500).json({
        erro:
          "Erro ao vincular usuário ao dispositivo.",
      });
    }
  }
  ...
  async getByDispositivo(
    ...
    try {
      ...
      const usuarios =
        await usuarioDispositivoService.getByDispositivo(
          dispositivoId,
        );
      ...
      return res.status(500).json({
        erro:
          "Erro ao buscar usuários do dispositivo.",
      });
    }
  }
  ...
  async delete(req: Request, res: Response) {
    try {
      ...
      await usuarioDispositivoService.delete(
        String(id),
      );
...
export const usuarioDispositivoController =
  new UsuarioDispositivoController();