...
enum Estados {
  ...
  ESTADO_ABASTECIMENTO_ABRINDO,
  ESTADO_ABASTECIMENTO_AGUARDANDO,
  ESTADO_VOLTAR_ABASTECIMENTO
};
...
void callback(char* topic, byte* payload, unsigned int length) {
  ...
  if (
    acao != NULL && String(acao) == "ABASTECER" && estadoAtual == ESTADO_IDLE) {

    Serial.printf(
      "[ABASTECIMENTO] Abrindo compartimento %d\n",
      compartimento);

    modoAbastecimento = true;

    compartimentoAberto = compartimento;

    prepararMovimento(compartimento);

    estadoAtual = ESTADO_ABASTECIMENTO_ABRINDO;
...
void loop() {
  ...
  switch (estadoAtual) {
    ...
    case ESTADO_ABASTECIMENTO_ABRINDO:
      ...
      if (passosRestantes == 0) {
        ...
        Serial.println(
          "[ABASTECIMENTO] Compartimento aberto");
    ...
    case ESTADO_ABASTECIMENTO_AGUARDANDO:

        
        if (millis() - cronometroEstado >= 5000) {

          Serial.println(
            "[ABASTECIMENTO] Finalizando"
          );
    ...
    case ESTADO_VOLTAR_ABASTECIMENTO:
